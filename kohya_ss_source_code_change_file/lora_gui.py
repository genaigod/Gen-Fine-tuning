...
def lora_train_api(
    ...
    try:
        ...
        with open(rf"training.txt", "w") as file:
            file.write(json.dumps({"task_id": celery_task_id, "status":"FAILED",  "msg":f"{traceback.format_exc()}"}))
        print(e)
    return 0
...
def custom_train_model(
    ...
    xxxx = {
        ...
        "sdxl_cache_text_encoder_outputs": sdxl_cache_text_encoder_outputs,
        "sdxl_no_half_vae": sdxl_no_half_vae,
        "full_bf16": full_bf16,
    ...
    log.info(f"838, default_configlora: {xxxx}")
    # Get list of function parameters and values
    parameters = list(locals().items())

    TRAIN_BUTTON_VISIBLE = None
...
def train_model(
    ...
    log.info(f"838, default_configlora: {default_config11}")
    # Get list of function parameters and values
    parameters = list(locals().items())
...
def lora_tab(
    ...
    with gr.Tab("Training"), gr.Column(variant="compact") as tab:
        ...
        gr.Interface(fn=is_train_lora, inputs=None, outputs="number", api_name="is_train_lora")
        celery_task_id = gr.Textbox(
            label="celery_task_id",
            placeholder="celery_task_id",
            interactive=True,
            value="",
        )
        gr.Interface(fn=lora_train_api, inputs=[celery_task_id, dummy_headless] + [dummy_db_true] + settings_list, outputs="number", api_name="lora_train_api")
