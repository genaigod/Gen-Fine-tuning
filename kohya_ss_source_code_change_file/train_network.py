...
if __name__ == "__main__":
    ...
    if os.path.exists("training.txt"):
        with open(r"training.txt", "r") as file:
            data111 = json.load(file)
    import traceback
    try:
        parser = setup_parser()
        ...
        with open(r"training.txt", "w") as file:
            file.write(json.dumps({"task_id": data111.get("task_id", "111"), "status":"SUCCESS","msg":f""}))


    except Exception as e:
        with open(r"training.txt", "w") as file:
            file.write(json.dumps({"task_id": data111.get("task_id", "111"), "status":"FAILED", "msg":f"{traceback.format_exc()}"}))
